...
public class UserServiceImpl implements IUserService {
    ...
    {
        //防止横向越权，校验当前用户的旧密码，一定要指定这个用户
        int resultCount = userMapper.checkPassword(MD5Util.MD5EncodeUtf8(passwordOld),user.getId());
        if(resultCount == 0)
        {
            return ServerResponse.createByErrorMessage("旧密码输入有误");
        }
    ...
    {
        //username 不能更新
        //email要校验是否已经存在，如果已经存在的话，所属用户不能为当前用户
       int resultCount = userMapper.checkEmailByUserId(user.getEmail(),user.getId());
       if(resultCount>0)
       {
           return ServerResponse.createByErrorMessage("email已经存在，请更换email再进行尝试");
       }
